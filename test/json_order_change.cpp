...
TEST_CASE("match", "[json_order_change]") {
  core::json::BufferStack buffer{8192, 1};
  json::OrderChange obj{MESSAGE_MATCH, buffer};
  /*
  CHECK(obj.type == json::Type::MESSAGE);
  CHECK(obj.topic == "/contractMarket/tradeOrders"sv);
  CHECK(obj.subject == json::Subject::ORDER_CHANGE);
...
TEST_CASE("match_parser", "[json_order_change]") {
  struct MyHandler final : public json::Parser::Handler {
    size_t count = {};
  ...
  core::json::BufferStack buffer{8192, 1};
  auto res = json::Parser::dispatch(handler, MESSAGE_MATCH, buffer, {}, false);
  CHECK(res == true);
  CHECK(handler.count == 1);
}
