...
TEST(json_order_change, example) {
  ...
  EXPECT_EQ(obj.topic, "/contractMarket/tradeOrders"_sv);
  EXPECT_EQ(obj.subject, json::Subject::ORDER_CHANGE);
  EXPECT_EQ(obj.channel_type, "private"_sv);
  auto &data = obj.data;
  EXPECT_EQ(data.order_id, "5cdfc138b21023a909e5ad55"_sv);
  EXPECT_EQ(data.symbol, "XBTUSDM"_sv);
  EXPECT_EQ(data.type, "match"_sv);
  EXPECT_EQ(data.status, "open"_sv);
  EXPECT_EQ(std::isnan(data.match_size), true);
  EXPECT_EQ(std::isnan(data.match_price), true);
  EXPECT_EQ(data.order_type, "limit"_sv);
  EXPECT_EQ(data.side, "buy"_sv);
  ...
  EXPECT_EQ(data.trade_id, "5ce24c16b210233c36eexxxx"_sv);
  EXPECT_EQ(data.client_oid, "5ce24c16b210233c36ee321d"_sv);
  EXPECT_EQ(data.order_time, 1545914149935808589ns);
  EXPECT_DOUBLE_EQ(data.old_size, 15000.0);
  EXPECT_EQ(data.liquidity, "maker"_sv);
